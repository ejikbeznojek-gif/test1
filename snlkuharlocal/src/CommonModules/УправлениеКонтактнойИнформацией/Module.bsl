...
Функция ОпределитьДляОбъектаРоссийскийАдрес(ОбъектРедактирования) Экспорт
	...
	Если (НЕ ПустаяСтрока(ОбъектРедактирования.Поле1)) Тогда
		...
		МожетБытьИностарнныйАдрес = ЕстьНеЦифры(ОбъектРедактирования.Поле1);
...
Функция ВидОбъектаКИ(ОбъектСсылка) Экспорт
	...
	ИначеЕсли ТипЗнч(ОбъектСсылка) = Тип("СправочникСсылка.ПунктыДоставки") Тогда
		Возврат Справочники.ВидыКонтактнойИнформации.АдресДоставкиКонтрагента;
	Иначе
...
//
Функция _СведенияОЮрФизЛице(ЮрФизЛицо, ДатаПериода, ДляФизЛицаТолькоИнициалы = Истина, БанковскийСчет = Неопределено) Экспорт
...
Процедура ИнициализацияКнопкиРедактированиеКИВДиалоге(Кнопка) Экспорт
	...
	РазрешеноВСписке = Ложь;//УправлениеДопПравамиПользователей.РазрешитьРедактированиеКИвСписке();
...
Процедура КонтактнаяИнформацияРедактироватьВДиалоге(Кнопка)Экспорт
	...
	РазрешеноВСписке = ПолучитьПолномочиеПользователя("РедактированиеКИвСписке");
