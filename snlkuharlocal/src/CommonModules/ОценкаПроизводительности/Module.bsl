Функция ПолучитьРазделительПути1()
	Возврат "\";
КонецФункции // ()

//Функция СтрЗаканчиваетсяНа(Стр,Хвост)
//	Возврат (Прав(СокрЛП(Стр),1)=Хвост);
//КонецФункции //СтрЗаканчиваетсяНа(Стр,Хвост)
...
Функция НачатьЗамерВремени() Экспорт
	...
	Если ОценкаПроизводительностиВызовСервераПовтИсп.ВыполнятьЗамерыПроизводительности() Тогда
		ВремяНачала = ОценкаПроизводительностиКлиентСервер.ПолучитьТекущуюУниверсальнуюДатуВМиллисекундах();
	КонецЕсли;
...
Процедура ЗакончитьЗамерВремени(КлючеваяОперация, ВремяНачала, ВесЗамера = 1, Комментарий = Неопределено) Экспорт
	...
	Если ОценкаПроизводительностиВызовСервераПовтИсп.ВыполнятьЗамерыПроизводительности() Тогда	
		ВремяОкончания = ОценкаПроизводительностиКлиентСервер.ПолучитьТекущуюУниверсальнуюДатуВМиллисекундах();
	...
	Попытка
		...
		Для Каждого КлючеваяОперация Из КлючевыеОперации Цикл
			...
			Если НЕ РезультатЗапроса.Пустой() Тогда
                ...
                КлючеваяОперацияОбъект.Наименование = РазложитьСтрокуВМассивПодстрок(КлючеваяОперация.ИмяКлючевойОперацииНовое,".")[0]; 
...
Процедура ЗафиксироватьЗамерДлительнойОперации(ОписаниеЗамера, КоличествоДанных, ИмяШага, Комментарий = "") Экспорт
	...
	ТекущееВремя = ОценкаПроизводительностиКлиентСервер.ПолучитьТекущуюУниверсальнуюДатуВМиллисекундах();
...
Функция ПериодЗаписи() Экспорт
	ТекущийПериод = ОценкаПроизводительностиПовтИсп.ПериодЗаписи();
...
Процедура ЗафиксироватьДлительностьКлючевойОперации(Параметры)
	...
	Запись.НомерСеанса = НомерСоединенияИнформационнойБазы();
	...
	Запись.ДатаЗаписи = Дата(1,1,1) + ОценкаПроизводительностиКлиентСервер.ПолучитьТекущуюУниверсальнуюДатуВМиллисекундах()/1000;
	...
	Запись.ДатаЗаписиЛокальная 	= ТекущаяДата();
...
Процедура ЭкспортОценкиПроизводительности(КаталогиЭкспорта, ДопПараметры = Неопределено) Экспорт
	...
	ТекДата = ОценкаПроизводительностиКлиентСервер.ПолучитьТекущуюУниверсальнуюДатуВМиллисекундах();
	Если ДопПараметры <> Неопределено Тогда
...
Процедура ОчиститьРегистрыЗамерыВремени() Экспорт
	...
	ГраницаУдаления = НачалоДня(ТекущаяДата() - 86400*Константы.ПериодХраненияЗамеров.Получить());
...
Процедура ВыгрузитьРезультаты(КаталогиЭкспорта, МассивыЗамеров, ТекДата, ПорядковыйНомерФайла)
	...
	ЗаписьXML.ЗаписатьАтрибут("period", ПространствоИмен, Строка(Дата(1,1,11) + ТекДата/1000));
