...
Функция НачатьЗамерВремениСлужебный(Параметры)
	...
	Возврат ПолучитьТекущуюУниверсальнуюДатуВМиллисекундах()/1000;
...
Функция ПолучитьТекущуюУниверсальнуюДатуВМиллисекундах() Экспорт
	#ЕСЛИ КЛИЕНТ ТОГДА
		Возврат ТекущаяУниверсальнаяДатаВМиллисекундах();
	#ИНАЧЕ
		Возврат ОценкаПроизводительностиВызовСервера.ТекущаяУниверсальнаяДатаВМиллисекундахНаСервере();		
	#КОНЕЦЕСЛИ
КонецФункции //ПолучитьТекущуюУниверсальнуюДатуВМиллисекундах()


#Область ЗамерыЕЕ

Процедура ЗаписатьЗамерПроизводительности_ПриЗаписи(Источник) Экспорт
	Если Источник = Неопределено Тогда Возврат КонецЕсли;
	
	МетаОбъект = Метаданные.НайтиПоТипу(ТипЗнч(Источник));
	Если МетаОбъект = Неопределено Тогда Возврат КонецЕсли; 
	
	Если НЕ (Метаданные.Документы.Содержит(МетаОбъект) ИЛИ Метаданные.Справочники.Содержит(МетаОбъект)) Тогда
		Возврат;	
	КонецЕсли; 

	Начало 		= МаркировкаКС.ПолучитьЗначениеПараметра(Источник.ДополнительныеСвойства, "Начало", 0);
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение ИЛИ РежимЗаписи = РежимЗаписиДокумента.ОтменаПроведения Тогда
		Возврат; //Это в подписке "Обработка проведения"	
	КонецЕсли;
	
	Если Начало > 0 Тогда 
		Если Источник.ДополнительныеСвойства.Свойство("Начало") Тогда
			Источник.ДополнительныеСвойства.Удалить("Начало");					
		КонецЕсли;
		//"2012-04-23T18:25:43.511Z"
		//Дата, Операция, ИмяТаблицы, УИД Ссылки, ВремяВыполнения, КолПоказатель, Доп показатели......
		
		Время			= (ТекущаяУниверсальнаяДатаВМиллисекундах() - Начало) / 1000;
		СообщениеЛога	= ""+Формат(ТекущаяДата(),"ДФ=yyyy-MM-ddTHH:mm:ss")+";ЗаписьОбъекта;"+МетаОбъект.ПолноеИмя()
			+";"+XMLСтрока(Источник.Ссылка)+";"+Формат(Время,"ЧДЦ=3")+";"+XMLСтрока(ВесОбъекта(Источник, МетаОбъект));
		Если ЗначениеЗаполнено(РежимЗаписи) Тогда
			СообщениеЛога = СообщениеЛога + ";ДП_РЗ:" + РежимЗаписи;					
		КонецЕсли;
		
		ЗаписатьВЛогПроизводительности(СообщениеЛога, Время);
	КонецЕсли;
КонецПроцедуры

Функция ВесОбъекта(Источник, МетаОбъект)
	Результат = 0;
	Если Метаданные.Документы.Содержит(МетаОбъект) Тогда
		Если МетаОбъект.ТабличныеЧасти.Найти("ШтрихкодыУпаковок") <> Неопределено Тогда
			Результат = Источник.ШтрихкодыУпаковок.Количество();		
		ИначеЕсли МетаОбъект.ТабличныеЧасти.Найти("Товары") <> Неопределено Тогда
			Результат = Источник.Товары.Количество();		
		КонецЕсли;	
	КонецЕсли;	
	
	Возврат Результат;
КонецФункции //ВесОбъекта(Источник)

Процедура ЗаписатьЗамерПроизводительности_ОбработкаПроведения(Источник) Экспорт
	Если Источник = Неопределено Тогда Возврат КонецЕсли;
	
	МетаОбъект = Метаданные.НайтиПоТипу(ТипЗнч(Источник));
	Если МетаОбъект = Неопределено Тогда Возврат КонецЕсли; 
	
	Если НЕ Метаданные.Документы.Содержит(МетаОбъект) Тогда Возврат КонецЕсли; 
	...
	Если Начало > 0 Тогда
		...
		КонецЕсли;
		//"2012-04-23T18:25:43.511Z"
		//Дата, Операция, ИмяТаблицы, УИД Ссылки, ВремяВыполнения, ВесОбъекта, Доп показатели......
...
Функция НачатьЗамерФункции(ИмяФункции, ДопПараметры = Неопределено) Экспорт	
	Возврат Новый Структура("ИмяФункции,Начало,ДопПараметры", ИмяФункции, ТекущаяУниверсальнаяДатаВМиллисекундах(), ДопПараметры);
КонецФункции //НачатьЗамерФункции(ИмяФункции) Экспорт

Функция ЗакончитьЗамерФункции(Замер, ВесОбъекта = 0) Экспорт
	Начало 		= МаркировкаКС.ПолучитьЗначениеПараметра(Замер, "Начало", 0);
	ИмяФункции	= МаркировкаКС.ПолучитьЗначениеПараметра(Замер, "ИмяФункции");
	ДопПараметры= МаркировкаКС.ПолучитьЗначениеПараметра(Замер, "ДопПараметры");
	
	Ссылка = МаркировкаКС.ПолучитьЗначениеПараметра(ДопПараметры, "Ссылка");
	
	Если Начало > 0 Тогда
		Время			= (ТекущаяУниверсальнаяДатаВМиллисекундах() - Начало) / 1000;		
		СообщениеЛога	= ""+Формат(ТекущаяДата(),"ДФ=yyyy-MM-ddTHH:mm:ss")+";ВызовФункции;"+ИмяФункции
			+";"+XMLСтрока(Ссылка)+";"+Формат(Время,"ЧДЦ=3")+";"+XMLСтрока(ВесОбъекта);
			
		ЗаписатьВЛогПроизводительности(СообщениеЛога, Время);		
	КонецЕсли;
КонецФункции //ЗакончитьЗамерФункции(Замер, ВесОбъекта = 0) Экспорт

Функция ЗаписатьВЛогПроизводительности(Сообщение, ВремяОбработки, ДопПараметры = Неопределено) Экспорт
	ТекСтр 		= ПараметрыСеанса.КэшЛогов;
	
	НовДлинна 	= СтрДлина(ТекСтр) + СтрДлина(Сообщение);
	
	Если НовДлинна > 50000 Тогда	
		ЗаписатьДанныеКэшаПроизвоДительности(ТекСтр);		
		ПараметрыСеанса.КэшЛогов = Сообщение;
	Иначе
		ПараметрыСеанса.КэшЛогов = ТекСтр + ?(ПустаяСтрока(ТекСтр), "", Символы.ПС) + Сообщение;			
	КонецЕсли;
КонецФункции //ЗаписатьВЛогПроизводительности(Сообщение, ВремяОбработки, ДопПараметры = Неопределено) Экспорт

Процедура ЗаписатьДанныеКэшаПроизвоДительности(ВходДанные) Экспорт
	Если ТипЗнч(ВходДанные) = Тип("Строка") Тогда
		Данные = ВходДанные;
		Если ПустаяСтрока(ВходДанные) Тогда Возврат КонецЕсли; 
	Иначе
		Данные = МаркировкаС.СериализоватьВJSON(ВходДанные);
	КонецЕсли; 
	...
	Попытка
		...
		Запись.КоличествоЗаписей   	= СтрЧислоВхождений(Данные, Символы.ПС) + 1;
		
		Запись.Автор 		= ПараметрыСеанса.Пользователь;
		Запись.ДатаЗаписи	= ТекущаяДата();
		
		Запись.НомерСеанса  	= НомерСеансаИнформационнойБазы();
		Запись.НомерСоединения	= НомерСоединенияИнформационнойБазы();
		
		Запись.Записать(Ложь);
	Исключение
		ЗаписатьВЛог("ЗамерыПроизводительности", "Ошибка записи замера. " + ОписаниеОшибки(), "Ошибка");
	КонецПопытки;
КонецПроцедуры

Процедура ЗаписатьКэшЛогов() Экспорт	
	ЗаписатьДанныеКэшаПроизвоДительности(ПараметрыСеанса.КэшЛогов);	
