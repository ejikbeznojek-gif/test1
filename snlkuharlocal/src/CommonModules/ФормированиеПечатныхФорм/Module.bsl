...
Функция СформироватьТекстНаименованияПлательшикаПолучателя(ТекстНаименования, ВладелецСчета, БанковскийСчет, ВБюджет = Ложь) Экспорт
	...
	//ВБюджет = (ВидОперации = Перечисления.ВидыОперацийППИсходящее.ПеречислениеНалога) ИЛИ ПеречислениеВБюджет;
	...
	Если ПустаяСтрока(ТекстРезультат) Тогда
		...
		ИначеЕсли ПустаяСтрока(БанковскийСчет.ТекстКорреспондента) Тогда
			...
			СведенияОВладельцеСчета = СведенияОЮрФизЛице(ВладелецСчета);
...
Функция СформироватьАвтоЗначенияРеквизитовПлательщикаПолучателя(Плательщик, СчетПлательщика, Получатель, СчетПолучателя, ВидОперации, ПеречислениеВБюджет = Ложь) Экспорт
	...
	УказаниеКППплательщикаОбязательно = Истина;  // ВБюджет ИЛИ СчетПлательщика.ВсегдаУказыватьКПП;
	...
	УказаниеКППполучателяОбязательно = Истина; //ВБюджет ИЛИ СчетПолучателя.ВсегдаУказыватьКПП;
	...
	НепрямыеРасчетыУПолучателя = Ложь; //ЗначениеЗаполнено(СчетПолучателя.БанкДляРасчетов);
	БанкПолучателя = 
