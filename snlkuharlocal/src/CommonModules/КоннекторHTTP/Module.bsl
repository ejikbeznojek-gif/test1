...
// * Тело - ДвоичныеДанные - 
// * Кодировка - Строка - 
// * ВремяВыполнения - Число, Неопределено - 
...
//
// Возвращаемое значение:
//   Соответствие Из Строка, Структура - ответ, десериализованный из JSON.
//     Параметры преобразования см. НовыеПараметры().ПараметрыПреобразованияJSON.
//
Функция GetJson(URL,
...
//    * Файлы - Массив Из См. НовыйОтправляемыйФайл - файлы, к отправке
...
// * ПараметрыЗапроса - Неопределено - 
// * ПроверятьSSL - Булево - 
// * КлиентскийСертификатSSL - Неопределено - 
// * Cookies - Соответствие Из Строка - 
// * Таймаут - Число - 
...
// ** ВариантЗаписиДатыJSON - ВариантЗаписиДатыJSON - 
// ** ИмяФункцииПреобразования - Неопределено - 
// ** МодульФункцииПреобразования - Неопределено - 
...
// * ПовторятьДляКодовСостояний - Неопределено - 
...
//    * Заголовки - Соответствие Из Строка - HTTP заголовки запроса.
//    * Аутентификация - Структура, Неопределено - параметры аутентификации запроса:
...
//                     Булево - Истина - используется значение СертификатыУдостоверяющихЦентровОС.
...
//    * СлужебныеДанные - Структура - служебные данные.
//
...
// Возвращаемое значение:
//  Структура:
//    * ИспользоватьАутентификациюОС - Строка, Булево - включает использование аутентификации NTLM или Negotiate.
//
...
// Конструктор аутентификации DigestНовая аутентификация digest.
// 
// Параметры:
//  Пользователь - Строка - Пользователь
//  Пароль - Строка - Пароль
...
//     * Путь - Строка, Число - адрес ресурса на сервере.
...
// ** Сервис - Строка - 
// ** Регион - Строка - 
// * Прокси - ИнтернетПрокси - 
// * ПараметрыЗапроса - Структура, Соответствие из Строка - 
// * Ключ - Строка - 
// * Значение - Строка, Массив из Строка - 
// * ПроверятьSSL - Булево, СертификатыУдостоверяющихЦентровФайл - 
// * КлиентскийСертификатSSL - СертификатКлиентаФайл, СертификатКлиентаWindows - 
// * МаксимальноеКоличествоПеренаправлений - Число - 
// * Cookies - Соответствие из Строка - 
//  Метод - Строка - Метод
//  URL - Строка - URL
...
// * ПроверятьSSL - Булево - 
// * КлиентскийСертификатSSL - СертификатКлиентаФайл, СертификатКлиентаWindows - 
// * Cookies - Массив из Строка - 
// * Таймаут - Число - 
// * РазрешитьПеренаправление - Булево - 
// * Json - Структура, Соответствие из Строка - 
// * ПараметрыПреобразованияJSON - Структура - :
// ** ПрочитатьВСоответствие - Булево - 
// ** ФорматДатыJSON - ФорматДатыJSON - 
// ** ИменаСвойствСоЗначениямиДата - Строка, Массив из Строка - 
...
// ** ИменаСвойствДляОбработкиВосстановления - Массив из Строка - 
// ** МаксимальнаяВложенность - Число - 
// * ПараметрыЗаписиJSON - ПараметрыЗаписиJSON - 
// * Данные - Строка, ДвоичныеДанные, Структура, Соответствие из Строка - :
// ** Ключ - Строка - 
// ** Значение - Строка - 
...
// * МаксимальноеВремяПовторов - Число - 
// * КоэффициентЭкспоненциальнойЗадержки - Число - 
// * ПовторятьДляКодовСостояний - Неопределено, Массив из Строка - 
// 
// Возвращаемое значение:
//  Структура - Вызвать HTTPМетод:
// * Метод - Строка - 
// * URL - Строка - 
// * КодСостояния - Число - 
// * Заголовки - Соответствие Из Строка - 
// * Тело - ДвоичныеДанные - 
// * Кодировка - Строка - 
// * ВремяВыполнения - Неопределено, Число - 
// * ЭтоПостоянныйРедирект - Булево - 
// * ЭтоРедирект - Булево - 
// * Cookies - Соответствие Из Строка - 
// * Ошибки - Массив Из Строка - 
...
Функция СоздатьПолеФормы(ИсходныеПараметры)
	...
	Поле.Вставить("Имя");
	Поле.Вставить("ИмяФайла");
	Поле.Вставить("Данные");
	Поле.Вставить("Тип");
	Поле.Вставить("Заголовки");
...
Функция ОтправитьЗапрос(Сессия, ПодготовленныйЗапрос, Настройки)
	...
	ПодготовленныйОтвет.Тело = Ответ.ПолучитьТелоКакДвоичныеДанные();Ответ.ПолучитьТелоКакСтроку();
...
Процедура ОбработкаОтветаСКодом401(Сессия, ПодготовленныйЗапрос, Настройки, Ответ)
	...
	Если Значение <> Ложь И СтрНайти(НРег(Значение), "digest") Тогда
		...
		ПараметрыDigest = Новый Структура;
		ПараметрыDigest.Вставить("algorithm");
		ПараметрыDigest.Вставить("realm");
		ПараметрыDigest.Вставить("nonce");
		ПараметрыDigest.Вставить("qop");
		ПараметрыDigest.Вставить("opaque");
...
Функция ПодготовитьЗаголовокDigest(Сессия, ПодготовленныйЗапрос)
	...
	Алгоритм1 = СтрШаблон("%1:%2:%3",
		ПодготовленныйЗапрос.Аутентификация.Пользователь,
		ПараметрыDigest.realm,
		ПодготовленныйЗапрос.Аутентификация.Пароль);
	Алгоритм2 = СтрШаблон("%1:%2", ПодготовленныйЗапрос.Метод, Путь);

	HA1 = ХешированиеДанных(Алгоритм, Алгоритм1);
	HA2 = ХешированиеДанных(Алгоритм, Алгоритм2);
...
#Иначе
	...
	ЧтениеФайлаАрхива = Новый ЧтениеФайлаАрхива(СжатыеДанные);
	ИмяФайла = ЧтениеФайлаАрхива.Элементы[0].Имя;
	Попытка
		ЧтениеФайлаАрхива.Извлечь(ЧтениеФайлаАрхива.Элементы[0], Каталог,
...
#Иначе
	...
	ПотокФайлаАрхива = Новый ПотокВПамяти;
	ЗаписьФайлаАрхива = Новый ЗаписьФайлаАрхива(ПотокФайлаАрхива);
	ЗаписьФайлаАрхива.Добавить(ВременныйФайл);
	ЗаписьФайлаАрхива.Записать();
	УдалитьФайлы(ВременныйФайл);

	Возврат ПотокФайлаАрхива.ЗакрытьИПолучитьДвоичныеДанные();
#КонецЕсли
