...
Процедура НачатьЗамерВремениНаКлиентеСлужебный(Параметры)
	...
	Если ПараметрыПриложения[ИмяПараметра] = Неопределено Тогда
		...
		Если ПараметрыПриложения[ИмяПараметраСтандартныеПодсистемы] = Неопределено Тогда
			...
			// Получаем дату в UTC
			ДатаИВремяНаКлиенте = ОценкаПроизводительностиКлиентСервер.ПолучитьТекущуюУниверсальнуюДатуВМиллисекундах();
	...
	Если АвтоЗавершение Тогда
		Если НЕ ПараметрыПриложения[ИмяПараметра]["ЕстьОбработчик"] Тогда
			...
			ПараметрыПриложения[ИмяПараметра]["ВремяПодключенияОбработчика"] = ОценкаПроизводительностиКлиентСервер.ПолучитьТекущуюУниверсальнуюДатуВМиллисекундах() + ПараметрыПриложения[ИмяПараметра]["СмещениеДатыКлиента"];
		КонецЕсли;	
	КонецЕсли;	
...
Процедура ЗавершитьЗамерВремениНаКлиентеАвто() Экспорт
	...
	Иначе
		...
		ОценкаПроизводительностиЗамерВремени["ВремяПодключенияОбработчика"] = ОценкаПроизводительностиКлиентСервер.ПолучитьТекущуюУниверсальнуюДатуВМиллисекундах() + ОценкаПроизводительностиЗамерВремени["СмещениеДатыКлиента"];
	КонецЕсли;
КонецПроцедуры
