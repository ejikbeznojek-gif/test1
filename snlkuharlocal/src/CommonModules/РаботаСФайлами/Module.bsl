...
// Ихсанов Д. А. Начало 11.03.2025 https://yougile.collant.ru/team/ab6bad3a05c0/#ID-17157
// Процедура - Внешняя обработка при открытии
//
// Параметры:
//  КодОбработки	 - Строка - Код обработки в справочнике Внешние отчеты
//  ВерсияОбработки	 - Число
//  Форма			 - Форма - Форма обработки из которой был вызван функционал
//
Процедура ВнешняяОбработкаПриОткрытии(КодОбработки, ВерсияОбработки, Форма) Экспорт 
	
	лВерсияОбработки = ПолучитьВерсиюОбработки(КодОбработки);
	Если лВерсияОбработки <> Неопределено Тогда
		ВерсияОбработки = лВерсияОбработки;
	КонецЕсли;
	
	Форма.Заголовок = СтрШаблон("Маркировка Озон v.%1", ВерсияОбработки);
	ПроверитьВерсиюОбработки(КодОбработки, ВерсияОбработки, Форма);
	
КонецПроцедуры // ВнешняяОбработкаПриОткрытии()

// Процедура - Обработка сравнения версии внешней обработки
//
// Параметры:
//  Этаформа				 - Форма - Форма обработки из которой был вызван функционал
//  ДополнительныеПараметры	 - Структура - ПолноеИмяВнешнегоОтчета, ВидОбработки
//  ИспользуемоеИмяФайла	 - Строка - Полное имя открытого файла внешней обработки
//
Процедура ОбработкаСравненияВерсииВнешнейОбработки(Этаформа, ДополнительныеПараметры, ИспользуемоеИмяФайла) Экспорт 
	
	Если ДополнительныеПараметры.ВидОбработки = "Обработка" Тогда
		Обработка = ВнешниеОбработки.Создать(ИспользуемоеИмяФайла);
	Иначе
		Обработка = ВнешниеОтчеты.Создать(ИспользуемоеИмяФайла);
	КонецЕсли;
	
	ФормаОбработки = Обработка.ПолучитьФорму("Форма",, Новый УникальныйИдентификатор);
	ЗаписатьФормированиеОтчета(ФормаОбработки, СтрШаблон("Внешний: %1", ДополнительныеПараметры.ПолноеИмяВнешнегоОтчета));
	ФормаОбработки.Открыть();
	Этаформа.Закрыть();
	
КонецПроцедуры // ОбработкаСравненияВерсииВнешнейОбработки()

// Процедура - Проверить версию обработки
//
// Параметры:
//  КодОбработки	 - См. РаботаСФайлами.ВнешняяОбработкаПриОткрытии()
//  ВерсияОбработки	 - См. РаботаСФайлами.ВнешняяОбработкаПриОткрытии()
//  Форма			 - См. РаботаСФайлами.ВнешняяОбработкаПриОткрытии()
//
Процедура ПроверитьВерсиюОбработки(КодОбработки, ВерсияОбработки, Форма) Экспорт
	
	пКаталогВнешнихОтчетов = Константы.КаталогВнешнихОтчетов.Получить();
	ПолноеИмяФайла = ПолноеИмяФайлаВерсииОбработки(КодОбработки, пКаталогВнешнихОтчетов);
	
	ФайлВерсии = Новый Файл(ПолноеИмяФайла);
	ТекстовыйФайлВерсии = Новый ТекстовыйДокумент;
	Если Не ФайлВерсии.Существует() Тогда
		ТекстовыйФайлВерсии.УстановитьТекст(ВерсияОбработки);
		ТекстовыйФайлВерсии.Записать(ПолноеИмяФайла);
		
		Возврат
	КонецЕсли;
	
	ТекстовыйФайлВерсии.Прочитать(ПолноеИмяФайла);
	ДанныеФайлаВерсии = Число(ТекстовыйФайлВерсии.ПолучитьТекст());
	Если ВерсияОбработки < ДанныеФайлаВерсии Тогда
		ЭлементВнешнегоОтчета = Справочники.ВнешниеОтчеты.НайтиПоКоду(КодОбработки);
		
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("ПолноеИмяВнешнегоОтчета", СтрШаблон("%1%2%3", пКаталогВнешнихОтчетов, 
			ЭлементВнешнегоОтчета.ИмяФайла, ?(ЭлементВнешнегоОтчета.ВидОбработки = "Обработка", ".epf", ".erf")));
		ДополнительныеПараметры.Вставить("ВидОбработки", 			ЭлементВнешнегоОтчета.ВидОбработки);
		
		ТекстПредупреждения = Нстр("ru = 'Внимание! Обнаружена обновленная версия обработки.
		|Сейчас форма закроется и откроется заново, если форма не открылась сама, то запустите обработку в ручном режиме!'");
		
		ОписаниеОповещенияОЗавершении = Новый ОписаниеОповещения("ОбработатьРезультатСравненияВерсииОбработки", Форма, 
			ДополнительныеПараметры);
		ПоказатьПредупреждение(ОписаниеОповещенияОЗавершении, ТекстПредупреждения, 5, Нстр("ru = 'Обновление обработки!'"));
	КонецЕсли;
	
КонецПроцедуры  // ПроверитьВерсиюОбработки()

// Функция - Получить версию обработки
//
// Параметры:
//  КодОбработки - Строка - Код обработки в справочнике Внешние отчеты
// 
// Возвращаемое значение:
//  Число - Текущая версия обработки из файла обработки
//
Функция ПолучитьВерсиюОбработки(КодОбработки) Экспорт
	...
	ТекстовыйФайлВерсии.Прочитать(ПолноеИмяФайла);
	Возврат Число(ТекстовыйФайлВерсии.ПолучитьТекст());
	
КонецФункции // ПолучитьВерсиюОбработки()

// Функция - Полное имя файла версии обработки
//
// Параметры:
//  КодОбработки			 - Строка - Код обработки в справочнике Внешние отчеты
//  пКаталогВнешнихОтчетов	 - Строка - Имя каталога с внешними отчетами
// 
// Возвращаемое значение:
//  Строка - Полное имя файла внешней обработки
//
Функция ПолноеИмяФайлаВерсииОбработки(КодОбработки, пКаталогВнешнихОтчетов)
	
	КаталогКФайламВерсии = СтрШаблон("%1%2", ?(Прав(пКаталогВнешнихОтчетов, 1) = "\", пКаталогВнешнихОтчетов, 
		пКаталогВнешнихОтчетов + "\"), "PrnForms\TempData\");
	Если Не РаботаСФайлами.ПроверитьСуществованиеКаталога(КаталогКФайламВерсии) Тогда
		Возврат Неопределено
	КонецЕсли;
	
	СтрокиПолноеИмяФайла = Новый Массив;
	СтрокиПолноеИмяФайла.Добавить(КаталогКФайламВерсии);
	СтрокиПолноеИмяФайла.Добавить(КодОбработки);
	СтрокиПолноеИмяФайла.Добавить(".txt");
	Возврат СтрСоединить(СтрокиПолноеИмяФайла);
	
КонецФункции // ПолноеИмяФайлаВерсииОбработки()
// Ихсанов Д. А. Конец  11.03.2025
