...
Процедура инт_КонтрольДлительныхОпераций() Экспорт
	
	АктивныеДлительныеОперации = инт_ДлительныеОперацииК.АктивныеДлительныеОперации();
	...
	Попытка
		инт_ПроконтролироватьДлительныеОперации(АктивныеДлительныеОперации.Список);
...
Процедура инт_ПроконтролироватьДлительныеОперации(АктивныеДлительныеОперации)
	...
	//Статусы = инт_ДлительныеОперации.ОперацииВыполнены(ЗаданияДляПроверки, ЗаданияДляОтмены);
	Статусы = инт_ДлительныеОперации.ОперацииВыполнены(ЗаданияДляПроверки);
	Для каждого СтатусОперации Из Статусы Цикл
		...
		Попытка
			Если инт_ПроконтролироватьДлительнуюОперацию(Операция, Статус) Тогда
	...
	ПодключитьОбработчикОжидания("инт_КонтрольДлительныхОпераций", Интервал, Истина);
	
КонецПроцедуры

Функция инт_ПроконтролироватьДлительнуюОперацию(ДлительнаяОперация, Статус)
	...
	Если Статус.Статус = "Выполнено" Тогда
		...
		инт_ДлительныеОперацииК.ПоказатьОповещение(ДлительнаяОперация.ОповещениеПользователя);
		инт_ВыполнитьОповещение(ДлительнаяОперация, Статус);
		Возврат Истина;
...
Процедура инт_ВыполнитьОповещение(Знач ДлительнаяОперация, Знач Статус)
