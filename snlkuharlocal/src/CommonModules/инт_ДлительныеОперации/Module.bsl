...
#КонецЕсли
	...
	Если ПараметрыВыполнения.Свойство("АдресРезультата") Тогда
		Если ПараметрыВыполнения.АдресРезультата = Неопределено Тогда
			Если Не ЗначениеЗаполнено(ПараметрыВыполнения.ИдентификаторФормы) И РежимОтладки() Тогда
				Попытка
				...
				Исключение
					...
					ЗаписьЖурналаРегистрации(НСтр("ru = 'Длительные операции.Диагностика'", КодОсновногоЯзыка()),
...
#Если ВнешнееСоединение Тогда
	ВыполнитьБезФоновогоЗадания = ИнформационнаяБазаФайловая 
		Или РежимОтладки() Или ПараметрыВыполнения.ЗапуститьНеВФоне
...
#Иначе
	ВыполнитьБезФоновогоЗадания = Не ПараметрыВыполнения.БезРасширений
		И (РежимОтладки() Или ПараметрыВыполнения.ЗапуститьНеВФоне
			Или (ЕстьФоновыеЗаданияВФайловойИБ() И Не ПараметрыВыполнения.ЗапуститьВФоне) 
			Или Не ВозможноВыполнитьВФоне(ИмяПроцедуры));
#КонецЕсли
	...
	УстановитьОтключениеБезопасногоРежима_(Истина);
	Попытка
	...
	Исключение
		...
		Возврат Результат;
	КонецПопытки;
	УстановитьОтключениеБезопасногоРежима_(Ложь);
...
Функция ЭтоФоновоеЗадание() Экспорт
	Результат = Ложь;
	ТекСеанс = ПолучитьТекСеанс();
	
	Возврат (ТекСеанс.ПолучитьФоновоеЗадание()<>Неопределено);	
КонецФункции //ЭтоФоновоеЗадание()

Функция ПолучитьТекСеанс()
	Результат = Неопределено;
	
	Сеансы = ПолучитьСеансыИнформационнойБазы();
	ТекСеанс = НомерСеансаИнформационнойБазы();
	Для Каждого Сеанс ИЗ Сеансы Цикл
		Если Сеанс.НомерСеанса = ТекСеанс Тогда
			Результат = Сеанс;
			Прервать;
		КонецЕсли;	
	КонецЦикла;
...
Процедура ОтменитьВыполнениеЗадания(Знач ИдентификаторЗадания) Экспорт 
	...
	ОтмененныеДлительныеОперации = МаркировкаКС.ПолучитьЗначениеПараметра(ОбщиеЗначения, "инт_ОтмененныеДлительныеОперации", Новый Соответствие);	
	Если ОтмененныеДлительныеОперации[ИдентификаторЗадания] = Неопределено Тогда
		ОтмененныеДлительныеОперации[ИдентификаторЗадания] = Истина;		
	КонецЕсли;
	ОбщиеЗначения["инт_ОтмененныеДлительныеОперации"] = ОтмененныеДлительныеОперации;
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Длительные операции.Отмена выполнения фонового задания'", КодОсновногоЯзыка()),
...
Функция ЗаданиеВыполнено(Знач ИдентификаторЗадания) Экспорт
	...
	Если Задание = Неопределено Тогда
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Длительные операции.Фоновое задание не найдено'", КодОсновногоЯзыка()),
	...
	ИначеЕсли Задание.Состояние = СостояниеФоновогоЗадания.Отменено Тогда
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Длительные операции.Фоновое задание отменено администратором'", КодОсновногоЯзыка()),
...
Функция ЗапуститьВыполнениеВФоне(Знач ИдентификаторФормы, Знач ИмяЭкспортнойПроцедуры, Знач Параметры,
	...
	Если РежимОтладки()
		...
		ВыполнитьМетодКонфигурации(ИмяЭкспортнойПроцедуры, ПараметрыЭкспортнойПроцедуры);
...
Функция ОперацияВыполнена(Знач ИдентификаторЗадания, Знач ИсключениеПриОшибке = Ложь, Знач ВыводитьПрогрессВыполнения = Ложь, 
	...
	Если Задание = Неопределено Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Длительные операции.Ошибка выполнения'", КодОсновногоЯзыка()),
	...
	Если Задание.Состояние = СостояниеФоновогоЗадания.Отменено Тогда
		УстановитьПривилегированныйРежим(Истина);
		ОбщиеПараметры = ПараметрыСеанса.ОбщиеЗначения.Получить();
		
		ОтмененныеДлительныеОперации = МаркировкаКС.ПолучитьЗначениеПараметра(ОбщиеПараметры, "ткд_ОтмененныеДлительныеОперации", Новый Соответствие);
...
Процедура ВыполнитьПроцедуруМодуляОбъектаОбработки(Параметры, АдресХранилища) Экспорт 
	...
	Если Параметры.ЭтоВнешняяОбработка Тогда
		Ссылка = МаркировкаКС.ПолучитьЗначениеПараметра(Параметры, "ДополнительнаяОбработкаСсылка");
		Если ЗначениеЗаполнено(Ссылка) И Ложь Тогда
	...
	ВыполнитьМетодОбъекта(Обработка, Параметры.ИмяМетода, ПараметрыМетода);
...
Функция ЗапуститьФоновоеЗаданиеСКонтекстомКлиента(ИмяПроцедуры,
	...
	Возврат ВыполнитьФоновоеЗадание(ПараметрыВыполнения,
		"инт_ДлительныеОперации.ВыполнитьСКонтекстомКлиента", ПараметрыПроцедурыФоновогоЗадания,
...
Процедура ВыполнитьСКонтекстомКлиента(ВсеПараметры) Экспорт
	...
	ЭтоФункция = МаркировкаКС.ПолучитьЗначениеПараметра(ВсеПараметры.ПараметрыВыполнения, "ЭтоФункция", Ложь);
	...
	Иначе
		ВызватьПроцедуру(ВсеПараметры.ИмяПроцедуры, ВсеПараметры.ПараметрыПроцедуры, ВсеПараметры.ПараметрыВыполнения);
	КонецЕсли;
...
Процедура ВызватьФункцию(ИмяФункции, ПараметрыПроцедуры, АдресРезультата)
	...
	Если Не ЭтоПроцедураМодуляОбработки Тогда
		Результат = ВызватьФункциюКонфигурации(ИмяФункции, ПараметрыПроцедуры);
		ПоместитьВоВременноеХранилище(Результат, АдресРезультата);
		Возврат;
	КонецЕсли;
	...
	Если ЭтоОбработка Или ЭтоОтчет Тогда
		...
		//Результат = ВызватьФункциюОбъекта(ОбработкаОтчетОбъект, ЧастиИмени[3], ПараметрыПроцедуры);
		ПоместитьВоВременноеХранилище(Результат, АдресРезультата);
		Возврат;
	КонецЕсли;
...
Процедура ВызватьПроцедуру(ИмяПроцедуры, ПараметрыВызова, ПараметрыВыполнения)
	...
	Если ЭтоОбработка Или ЭтоОтчет Тогда
		МенеджерОбъекта = ?(ЭтоОтчет, Отчеты, Обработки);
		ОбработкаОтчетОбъект = МенеджерОбъекта[ЧастиИмени[1]].Создать();
		ВыполнитьМетодОбъекта(ОбработкаОтчетОбъект, ЧастиИмени[2], ПараметрыВызова);		
	ИначеЕсли ЭтоВнешняяОбработка Или ЭтоВнешнийОтчет Тогда
		ОбработкаОтчетОбъект = ВнешняяОбработкаОтчетОбъект(ЭтоВнешнийОтчет, ПараметрыВыполнения, ЧастиИмени[1]);
		ВыполнитьМетодОбъекта(ОбработкаОтчетОбъект, ЧастиИмени[2], ПараметрыВызова);		
	ИначеЕсли НЕ ЭтоПроцедураМодуляОбработки Тогда
...
Функция УстановитьОтключениеБезопасногоРежима_(Отключить)
...
Функция ЗначениеВСтрокуXML(Значение) Экспорт
	...
	Если ТипЗнч(Значение) = Тип("ХранилищеЗначения") Тогда
		//Оборачивание ХранилищаЗначения
		СериализаторXDTO.ЗаписатьXML(ЗаписьXML, Новый ХранилищеЗначения(Значение));
	Иначе
		Попытка
...
Функция ЗначениеИзСтрокиXML(СтрокаXML) Экспорт
	...
	Если ТипЗнч(ХранилищеЗначение) = Тип("ХранилищеЗначения") Тогда
...
Функция ВызватьФункциюКонфигурации(Знач ИмяМетода, Знач Параметры = Неопределено) Экспорт
	...
	рВыполнения = Вычислить(ИмяМетода + "(" + ПараметрыСтрока + ")"); // АПК:488 Исполняемый код безопасен.
	...
	Если ФЗ <> Неопределено Тогда
		РегистрыСведений.ОчередьЗадач.ОбновитьСостояниеИсполняющегоФЗ(ФЗ, рВыполнения);	
	КонецЕсли; 
...
Процедура ПроверитьИмяПроцедурыКонфигурации(Знач ИмяПроцедуры)
	...
	Если ЧастиИмени.Количество() = 3 Тогда
		...
		Попытка
			Менеджер = Вычислить(ПолноеИмяОбъекта);
		Исключение
...
Процедура ЗаписьЖурналаРегистрации_(ИмяСобытия, Знач Уровень, Знач ОбъектМетаданных, Данные, Комментарий, РежимТранзакции = Неопределено) Экспорт 
	
	Если Уровень = "Информация" Тогда
	...
	ИначеЕсли Уровень = "Примечание" Тогда
...
КонецПроцедуры //ЗаписьЖурналаРегистрации(ИмяСобытия, Знач Уровень, Знач ОбъектМетаданных, Данные, Комментарий, РежимТранзакции = Ложь) Экспорт

Функция НайтиФоновыеЗадания(ИмяМетода, ДопПараметры = Неопределено) Экспорт	
	Отбор = Новый Структура("ИмяМетода", ИмяМетода);
	
	Для Каждого ПолеФильтра Из СтрРазделить("Ключ,Состояние,УникальныйИдентификатор", ",") Цикл
		ЗначФильтра = МаркировкаКС.ПолучитьЗначениеПараметра(ДопПараметры, ПолеФильтра);
		Если ЗначениеЗаполнено(ЗначФильтра) Тогда
			Отбор.Вставить(ПолеФильтра, ЗначФильтра);	
		КонецЕсли;	
	КонецЦикла;
	
	Возврат ФоновыеЗадания.ПолучитьФоновыеЗадания(Отбор);	
КонецФункции //НайтиФоновоеЗадание(ИмяМетода, ДопПараметры = Неопределено) Экспорт

Функция ОжидатьЗавершенияЗаданий(Задания, МаксЧислоФЗ = 0) Экспорт
	АктивныеФЗ = ?(ТипЗнч(Задания) = Тип("Массив"), Задания, Новый Массив);
	Пока АктивныеФЗ.Количество() > Макс(0, МаксЧислоФЗ) Цикл
	...
	Возврат АктивныеФЗ;
КонецФункции //ОжидатьЗавершенияЗаданий(Задания, МаксЧислоФЗ = 0) Экспорт

Функция ОписаниеОшибкиФЗ(Задание) Экспорт
	ТекстОшибки = "";
	ОшибкаЗадания = Задание.ИнформацияОбОшибке;
	Если ОшибкаЗадания <> Неопределено Тогда
		ТекстОшибки = СтрШаблон("%1. Ошибка выполнения ФЗ %2 (%3). 
		|Причина %4.",
	...
	КонецЕсли;
    Возврат ТекстОшибки;
КонецФункции //ОписаниеОшибкиФЗ(Задание) Экспорт
