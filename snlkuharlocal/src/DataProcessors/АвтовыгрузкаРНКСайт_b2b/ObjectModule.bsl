...
Процедура ОчиститьСписокРНКНаВыгрузку(СписокЗаказов=неопределено, СписокЗаказовРНК=неопределено)
	...
	//Для каждого текстрока из СписокЗаказов цикл
	//	
	//	// тут проверку, что все РНК отгружены?
	//	Докссылка = ТекСтрока.Заказссылка;
	//	попытка
	...
	// и все РНК переведем в отгружено, которые мы отправили.
...
Процедура Сформировать(ТаблицаПараметров=НЕОПРЕДЕЛЕНО) Экспорт
	...
	ЗаписатьВЛог("b2b","Начало выполнения. Задача: 1 обмен заказами. !","Информация");
	...
	ЗаписатьВЛог("b2b","Конец выполнения. Задача: 3 Обмен заказами. !","Информация");                     
