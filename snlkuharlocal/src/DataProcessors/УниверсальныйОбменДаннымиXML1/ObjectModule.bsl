	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		Если ИмяУзла = "Свойство"
			...
			Иначе
				...
				Иначе
					...
					Попытка
						КонвертацияИсточник = Конвертация.Источник;
					Исключение
						КонвертацияИсточник = "";
					КонецПопытки;
					Попытка
		...
		ИначеЕсли (ИмяУзла = "Объект") И (ФайлОбмена.ТипУзла = одТипУзлаXML_КонецЭлемента) Тогда
			...
			ИначеЕсли ИмяТипаОбъекта <> "Перечисление" Тогда
				...
				//Катаев+ 07/04/14 Справочники в любом случае изменять нельзя
				Если Лев(ТипОбъекта,10) = "Справочник" И НужноЗаписатьОбъект Тогда
					НужноЗаписатьОбъект = Ложь;
				КонецЕсли;
				//Катаев- 07/04/14
				Если НужноЗаписатьОбъект Тогда
