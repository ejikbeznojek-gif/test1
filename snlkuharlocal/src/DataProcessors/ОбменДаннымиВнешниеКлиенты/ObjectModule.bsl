...
Процедура Сообщить1(стр) экспорт
	...
	#ЕСЛИ КЛИЕНТ ТОГДА
    Сообщить(стр);
	ЗаписатьВЛог("Выгрузка данных Внешние",стр,Перечисления.ТипыСообщенийЛога.Информация); 
 	#ИНАЧЕ
	ЗаписатьВЛог("Выгрузка данных Внешние",стр,Перечисления.ТипыСообщенийЛога.Информация); 
	#КОНЕЦЕСЛИ
	...
	Если Узлы.Количество()=0 тогда
		Если ТаблицаПараметров2<>Неопределено Тогда
			Для Каждого СтрокаТП ИЗ ТаблицаПараметров2 Цикл
				ЗаписатьВЛог("Выгрузка данных Внешние",""+СтрокаТП.Значение+" тип= "+ТипЗнч(СтрокаТП.Значение),"Информация");
				
				Если СтрокаТП.Имя="Узел" Тогда
					текУзел=(СтрокаТП.Значение);
					//крохин начало 30.09.2020
			ТекВнешнийУзел =  ПланыОбмена.ВнешниеКлиенты.НайтиПоКоду(текУзел);
			Если ТекВнешнийУзел.ПометкаУдаления Тогда Продолжить; КонецЕсли;
			//крохин конец 30.09.2020

					//Катаев+ 16/06/14
					//КнопкаЗагрузить(текузел);
					Если Не ПланыОбмена.ВнешниеКлиенты.НайтиПоКоду(текУзел).НеПроизводитьЗагрузку Тогда
						КнопкаЗагрузить(текузел);
					КонецЕсли;
					//Катаев- 16/06/14
					КнопкаВыгрузить(текузел);
				Конецесли;
			Конеццикла;
		Конецесли;
	иначе
		Для Каждого СтрокаТП ИЗ Узлы Цикл
			
			//крохин начало 30.09.2020
			ТекВнешнийУзел =  ПланыОбмена.ВнешниеКлиенты.НайтиПоКоду(СтрокаТП.узел);
			Если ТекВнешнийУзел.ПометкаУдаления Тогда Продолжить; КонецЕсли;
			//крохин конец 30.09.2020
			
			//Катаев+ 16/06/14
			//КнопкаЗагрузить(СтрокаТП.узел);
			Если Не ПланыОбмена.ВнешниеКлиенты.НайтиПоКоду(СтрокаТП.узел).НеПроизводитьЗагрузку Тогда
				КнопкаЗагрузить(СтрокаТП.узел);
			КонецЕсли;
			//Катаев- 16/06/14
			КнопкаВыгрузить(СтрокаТП.узел);
...
Процедура КнопкаВыгрузить(Узел=НЕОПРЕДЕЛЕНО) Экспорт
	...
	для ном=0 по 50 цикл
		...
		//Катаев+ 27/05/14
		//крохин начало 16.10.2017
		//ИмяфайлаНаСервере = "\\srvmail2\ftpfolders$\exch80\snl\"+СтрНастроек.ИмяФайлаЗаписиИзменений+сим+Формат(ТекущаяДата(), "ДФ=ггММддЧЧммсс")+".zip";
		//крохин начало 14.10.2020
		//новый ftp-сервер
		//ИмяфайлаНаСервере = "\\srvftp\ftp_1c$\SNL\"+СтрНастроек.ИмяФайлаЗаписиИзменений+сим+Формат(ТекущаяДата(), "ДФ=ггММддЧЧммсс")+".zip";
			
		// Кухарь А.Ю.  18.01.2023 начало
		//ИмяфайлаНаСервере = "\\srvftp-c\c$\FTP_Folder\1C\snl\"+СтрНастроек.ИмяФайлаЗаписиИзменений+сим+Формат(ТекущаяДата(), "ДФ=ггММддЧЧммсс")+".zip";	
		ИмяфайлаНаСервере = "\\srvftp-c\FTP_Folder$\1C\snl\"+СтрНастроек.ИмяФайлаЗаписиИзменений+сим+Формат(ТекущаяДата(), "ДФ=ггММддЧЧммсс")+".zip";	 
		// Кухарь А.Ю.  18.01.2023 конец
		
		//крохин конец 14.10.2020
		//крохин конец 16.10.2017
		//Катаев- 27/05/14
	...
	РезультатАрхивирования = заархивироватьфайл2(ИмяфайлаОбмена,ИмяФайлаАрхива);
	//Катаев- 29/05/14
	
	//Катаев+ 27/05/14 Отказ от FTP
	//КопироватьФайл2(ИмяфайлаАрхива,ПолучитьСтрокуОтделеннойСимволом(ИмяфайлаАрхива,"\"),Соединение,ИСТИНА);
	КопироватьФайл(ИмяфайлаАрхива, ИмяфайлаНаСервере);
	//Катаев- 27/05/14
//	Сообщить1("Выгружено...");
	//Катаев+ 27/05/14 Очищаем изменнения в узле
	Если РезультатАрхивирования ТОгда
		//Катаев+ 27/06/14
		Сообщить("Файл обмена помещен на ftp сервер. Имя файла "+ИмяфайлаНаСервере);
		//Катаев- 27/06/14
...
Процедура КнопкаЗагрузить(Узел=неопределено) Экспорт
	...
	Конецесли;
		...
		ЗадатьНастройки(Узел);
        //Катаев+ 27/05/14 Отказ от подключения через FTP
		//МассивФайлов=Соединение.найтифайлы("",СтрНастроек.ИмяФайлаЧтенияИзменений+"*");
		//крохин начало 16.10.2017
		//МассивФайлов = НайтиФайлы("\\srvmail2\ftpfolders$\exch80\snl\",СтрНастроек.ИмяФайлаЧтенияИзменений+"*");
		МассивФайлов = НайтиФайлы("\\srvftp\ftp_1c$\SNL\",СтрНастроек.ИмяФайлаЧтенияИзменений+"*");
		//крохин конец 16.10.2017
		//Катаев- 27/05/14
		Для каждого файл1 из МассивФайлов цикл
			...
			КопироватьФайл(файл1.ПолноеИмя, ИмяФайлаОбмена);
			//Катаев- 27/05/14
...
Процедура задатьнастройки(Узел=НЕОПРЕДЕЛЕНО)
...
//крохин начало 01.10.2020
 //СерверFTP="188.65.108.67";
 СерверFTP="87.239.110.105";
 //крохин конец 01.10.2020
//крохин конец 16.10.2017
...
нов.ИмяФайлаЧтенияИзменений="Torg"+Сокрлп(узел.код);
нов.ИмяФайлаЗаписиИзменений="Vnesh_"+Сокрлп(узел.код);
