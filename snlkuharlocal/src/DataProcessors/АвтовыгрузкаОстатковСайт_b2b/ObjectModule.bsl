...
//	Склады.Добавить(справочники.Склады.НайтиПоКоду("STA-48"));
	...
	Запрос=Новый Запрос;     
		...
		запрос.Текст = "ВЫБРАТЬ
	               ...
	               |			ТОГДА 1000
	               |		ИНАЧЕ ВЫБОР
	               |				КОГДА СУММА(ВложенныйЗапрос.КолКонФакт) есть NULL
	               ...
	               |	СУММА(ВложенныйЗапрос.КолКонБух) КАК КолКонБух,
	               |	СУММА(ВложенныйЗапрос.Резерв) КАК Резерв,
	           	   |	Ном.Ссылка.родитель.Код как моделькод,
	               |	Ном.Код КАК Код,
	               |	Ном.Наименование КАК Наименование
	               |ИЗ
	               |	Справочник.Номенклатура КАК Ном
	...
	Склады.Добавить(справочники.Склады.НайтиПоКоду("STC-60"));
	Склады.Добавить(справочники.Склады.НайтиПоКоду("STA-48"));
	Склады.Добавить(справочники.Склады.НайтиПоКоду("STD-13"));
...
//	
	...
	Пока Рез.Следующий() Цикл
		...
		Для каждого текстрока из рез.ссылка.контрагенты цикл
			...
			//нечаева 01,03,23+++
			Если Текстрока.контрагент.ведущий = null Тогда
				продолжить;
			иначе
